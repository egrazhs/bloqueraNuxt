<template>
	<section>
		...
		<div class="mt-4">
			<NuxtLink to="./sucursales/create" class="py-2 px-4 bg-green-600 hover:bg-green-700 text-white rounded-xl text-xs">Agregar nuevo Usuario</NuxtLink>
		</div>
		...
		<table class="w-full text-xs">
			...
			<tbody class="border-1 border-black pl-2">
				<tr v-for="sucursal in sucursales" class="odd:bg-slate-200 even:bg-slate-50 hover:bg-slate-300">
					...
					<td class="py-2 pr-2">
						<NuxtLink :to="`./sucursales/read/${sucursal.id}`" class="p-2 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded-md">Ver Más</NuxtLink>
						<NuxtLink :to="`./sucursales/edit/${sucursal.id}`" class="p-2 ml-2 text-xs bg-orange-600 hover:bg-orange-700 text-white rounded-md text-slate-20">Editar</NuxtLink>
