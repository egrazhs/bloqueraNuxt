	...
	<v-container>
		<v-card class="mx-auto mt-5" max-width="600">
			<v-card-title class="headline d-flex align-center">
				...
				<span class="ml-4">Editar Sucursal</span>
			</v-card-title>
			<v-divider></v-divider>
			<v-card-text>
...
<script setup>
	...
	const fetchDoc = async () => {
		try {
			...
			const doc_a_editar = await getDoc(doc(db, 'sucursales', id));
			...
			console.error('Error al obtener detalles de la sucursal:', error.message);
		} finally {
			loading.value = false;
		}
	};
	...
	const guardar = async () => {
		try {
			const { id } = route.params;
			await updateDoc(doc(db, 'sucursales', id), documento.value);
			router.push({ path: '/sucursales', query: { documentoActualizado: true } });
		} catch (error) {
