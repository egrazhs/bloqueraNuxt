		...
		<v-card v-if="doc_info" class="mx-auto mt-5">
			<v-card-title class="headline d-flex align-center justify-between">
				<div>
					...
					<span class="ml-4">Detalles de la familia de Producto</span>
				</div>

				<ButtonsEdit :route="'/familias_productos/edit/' + doc_info.id" />
			</v-card-title>
...
<script setup>
	...
	onMounted(async () => {
		try {
			...
			const data = await getDoc(doc(db, 'familias_productos', id));
			if (data.exists()) {
